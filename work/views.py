...
def job_costing(request, begin, end,full):
    ...
    property_set = Property.objects.filter(job_costing_report_include=True).exclude(client_name__end_date__lte=begin).exclude(end_date__lte=begin).order_by('client_name', 'display_name').prefetch_related(Prefetch('location',queryset=job_set,to_attr='jobs')).prefetch_related(Prefetch('location__job_location__client_name'))
    if full == 'all':
...
class AnnualSchedule(LoginRequiredMixin,generic.ListView):
    def get_queryset(self):
        queryset = Property.objects.filter(times_per_year__gt=0).filter(check_priority__lt=3).prefetch_related('location').prefetch_related('location__job_shift').prefetch_related('location__job_shift__driver')
...
class UpdateProperty(LoginRequiredMixin,generic.UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        obj.last_ten = obj.location.all().order_by('-job_shift__date')[:31].prefetch_related('job_shift').prefetch_related('job_shift__driver')
...
def pdf_build(shift):
    ...
    for job in shift.jobs_in_shift.all():
        ...
        style_t = ParagraphStyle('jobs',fontName='Helvetica',fontSize=8,borderPadding=(3,5,3,5))
...
def landscape_pdf_build(shift):
    ...
    # Start writing the PDF here
    p = SimpleDocTemplate(buffer, leftMargin=0.5*inch,rightMargin=0.5*inch,bottomMargin=0.5*inch,topMargin=0.5*inch,pagesize=landscape(letter))
    logo=os.path.join(settings.BASE_DIR,'static','mobs','landscaping.jpg')
    I = Image(logo,width=2.2*inch,height=1.2*inch)
    # container for the 'Flowable' objects
    elements = []
    # Header table
    data_h = [[I,shift.date.strftime('%A')+': '+shift.date.strftime('%b %d, %Y'),'',shift.driver,'Weather'],
                ['','Driver','Time in','Time out','Viento (Windy)',''],
                ['','Helper','Time in','Time out','Lloviendo (Rain)',''],
                ['','Helper','Time in','Time out','',''],
                ['','Lunch (Almuerzo)','Time in','Time out','',''],
                ['','Truck #','Mileage in','Mileage out',''],]
    ...
    t_h.setStyle(TableStyle([('GRID',(1,1),(3,5),1,colors.black),
                            ('GRID',(5,1),(5,2),1,colors.black),
                            ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
                           ('FONT',(0,1),(-1,-1),'Helvetica',8),
                           ('FONT',(0,0),(-1,0),'Helvetica-Bold',8),
                           ('SPAN',(0,0),(0,-1),
