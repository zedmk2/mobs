...
def job_costing(request, begin, end,full):
    ...
    property_set = Property.objects.filter(job_costing_report_include=True).exclude(client_name__end_date__lte=begin).order_by('client_name', 'display_name').prefetch_related(Prefetch('location',queryset=job_set,to_attr='jobs')).prefetch_related(Prefetch('location__job_location__client_name'))
    if full == 'all':
