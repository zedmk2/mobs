...
for prop in jc:
    if prop['inv_type'] != 'NI':
        ...
        if prop['inv_type'] == 'RL':
            if prop['adlspl']:
                spl_qty = float(prop['adlspl']) * -1
                neg_amount = spl_qty * prop['sw_price']
                amount = neg_amount*qty
            ...
            spl = ['SPL', '', 'INVOICE', date, 'Sales:Sweeping',
                '', neg_amount,'', prop['memo'], 'N', spl_qty, prop['sw_price'], 'Sweeping',
                '', 'N', 'N', 'NOTHING', service_date, '', '',
                '', '', '', '', '']
            for a in range(rl_qty):
                iif_writer.writerow(spl)

        if prop['inv_type'] == 'Custom':
            ...
            if prop['saddr1'] == 'Village at Waugh Chapel':
                print('village')
                neg_amount = prop['sw_mo_price'] *-1
                amount = 4615.6
                duedate = net30date
                trans = ['TRNS', '', 'INVOICE', date, 'Accounts Receivable',
                        prop['client_name'], amount, docnum, '', 'N', 'Y', 'N', prop['addr1'],
                        ...
                        prop['saddr1'], prop['saddr2'], prop['saddr3'], prop['saddr4'], prop['tosend']]
                spl = ['SPL', '', 'INVOICE', date, 'Sales:Sweeping',
                    '', neg_amount,'', prop['memo'], 'N', qty, prop['sw_mo_price'], 'Sweeping',
                    '', 'N', 'N', 'NOTHING', service_date, '', '',
                    '', '', '', '', '']
                iif_writer.writerow(trans)
                iif_writer.writerow(spl)
                spl[4] = 'Sales:Dumpster'
                spl[6] = -477.27
                spl[8] = 'Dumpster fee'
                spl[11] = 477.27
                spl[12] = 'Dumpster'
...
iif_file.close()
copyfile('output_for_qb.csv','output_for_qb.iif')
print('!!break!!')
